  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-slide-up">
      <Link href={`/meetings/${meetingId}`}>
        <div className="group flex items-center gap-3 rounded-2xl border border-emerald-500/25 bg-[#09090b]/90 px-4 py-3 shadow-lg shadow-emerald-500/10 backdrop-blur-xl transition-all duration-200 hover:border-emerald-500/40 hover:shadow-emerald-500/20">
          <span className="relative flex h-2.5 w-2.5">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />
            <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-emerald-500" />
          </span>
          <div className="flex flex-col">
            <span className="text-sm font-medium text-zinc-100 group-hover:text-emerald-300 transition-colors">
              Đang trong meeting
            </span>
